...
// Copy the video file directly into public/funded-futures-video.mp4.
...
function NavDropdown({
  ...
  function renderItem(item: NavItem, idx: number) {
    if (item.to) {
      return (
        <Link key={idx} to={item.to} className={itemClass} onClick={() => setIsOpen(false)}>
          {item.label}
        </Link>
      );
    }
    return (
      <a key={idx} href={item.href} className={itemClass} onClick={() => setIsOpen(false)}>
        {item.label}
      </a>
    );
  }
      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex flex-1 ml-12 justify-between items-center bg-white/50 backdrop-blur-sm rounded-full px-2">
            ...
            <button onClick={openContact} className="flex-1 text-center font-medium hover:text-forest-green transition-colors py-3 cursor-pointer">
              Contact Us
            </button>
            <div className="flex-1 flex justify-center px-2">
              <Button variant="primary" className="w-full py-2 text-xs uppercase tracking-widest truncate" onClick={() => navigate('/donate')}>
                Donate
              </Button>
            </div>
          </div>
          ...
          {isMenuOpen && (
            ...
            >
              <div className="flex flex-col p-6 gap-6">
                <Link to="/" className="text-lg font-medium" onClick={() => { setIsMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
                  Home
                </Link>
                ...
                <button onClick={() => { setIsMenuOpen(false); openContact(); }} className="text-lg font-medium text-left">
                  Contact Us
                </button>
                <Button variant="primary" onClick={() => { setIsMenuOpen(false); navigate('/donate'); }}>
                  Donate Now
                </Button>
              </div>
            </motion.div>
      ...
      <main>
        ...
        <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-off-white">
          ...
          <div className="relative z-10 text-center px-6 max-w-4xl py-20">
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-4xl sm:text-5xl md:text-7xl font-display font-bold leading-tight mb-6 text-deep-slate">
              A better world begins in the mind of a child
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="text-lg sm:text-xl md:text-2xl text-deep-slate/80 mb-10 max-w-2xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Button variant="primary" className="flex items-center justify-center gap-2 w-full sm:w-auto" onClick={() => navigate('/donate')}>
                Support a Student <ArrowRight size={18} />
              </Button>
              <Button variant="muted" className="w-full sm:w-auto" onClick={() => navigate('/learn-story')}>
                Learn Our Story
              </Button>
            </motion.div>
          </div>
          <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} className="absolute top-1/4 left-10 w-20 h-20 bg-forest-green/10 rounded-full blur-2xl" />
          <motion.div animate={{ y: [0, 20, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: 1 }} className="absolute bottom-1/4 right-10 w-32 h-32 bg-frosted-blue/20 rounded-full blur-3xl" />
        </section>

        <section id="story-video" className="py-16 md:py-24 px-[5%] bg-white">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-5xl mx-auto">
            <div className="text-center mb-10">
              <p className="text-forest-green font-bold text-sm tracking-widest uppercase mb-4">See the difference</p>
              <h2 className="text-4xl md:text-5xl font-display font-medium">Every child deserves a chance to learn</h2>
              <p className="text-base md:text-lg text-muted-text max-w-2xl mx-auto mt-5">
                Watch how access to education helps children protect their potential and shape a better future.
              </p>
            </div>
            <div className="rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-deep-slate border border-forest-green/10">
              {HOMEPAGE_VIDEO_URL ? (
                <video className="w-full aspect-video object-cover" controls playsInline preload="metadata" poster={HOMEPAGE_VIDEO_POSTER} aria-label="fundED futures education story">
                  <source src={HOMEPAGE_VIDEO_URL} type="video/mp4" />
                  Your browser does not support embedded video.
                </video>
              ) : (
                <div className="aspect-video flex flex-col items-center justify-center text-center text-white px-6">
                  <div className="w-16 h-16 rounded-full bg-forest-green flex items-center justify-center mb-5"><Play size={28} fill="currentColor" /></div>
                  <h3 className="text-2xl font-display font-bold mb-3">Our story in motion</h3>
                  <p className="max-w-md text-white/70 text-sm leading-relaxed">Add the direct MP4 or WebM link to HOMEPAGE_VIDEO_URL in App.tsx to show the video here.</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        <section id="our-mission" className="py-20 md:py-32 px-[5%] bg-[#f7fcfb]">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative aspect-square md:aspect-[4/5] rounded-3xl overflow-hidden border border-gray-100 order-2 md:order-1">
              <img src="https://i.imgur.com/c2tmG1v.jpeg" alt="Community context" className="w-full h-full object-cover hover:scale-105 transition-smooth duration-1000" referrerPolicy="no-referrer" />
              <div className="absolute inset-0 bg-gradient-to-t from-deep-slate/40 to-transparent" />
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="space-y-6 text-center md:text-left order-1 md:order-2">
              <h2 className="text-4xl md:text-6xl leading-tight">Our Mission</h2>
              <p className="text-base md:text-lg leading-relaxed opacity-90">Education is a fundamental right, yet for thousands of students across Kenya, financial barriers make schooling an impossible dream. fundED futures was born from a simple belief: that poverty should never be a barrier to potential.</p>
              <p className="text-base md:text-lg leading-relaxed opacity-90">providing not just school fees, but the uniforms, books, and materials they need to thrive.</p>
              <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-6 w-full">
                <Button onClick={() => navigate('/impact-areas')} className="flex items-center justify-center gap-4 bg-white !text-deep-slate border border-forest-green/10 !py-6 !rounded-[2rem] hover:bg-forest-green/5">
                  <div className="w-10 h-10 bg-forest-green/10 rounded-full flex items-center justify-center text-forest-green"><HandHeart size={20} /></div>
                  <span className="font-bold text-base">Direct Impact</span>
                </Button>
                <Button onClick={() => navigate('/programs')} className="flex items-center justify-center gap-4 bg-white !text-deep-slate border border-frosted-blue/20 !py-6 !rounded-[2rem] hover:bg-frosted-blue/10">
                  <div className="w-10 h-10 bg-frosted-blue/20 rounded-full flex items-center justify-center text-frosted-blue"><BookOpen size={20} /></div>
        ...
        <section id="how-it-works" className="py-20 md:py-32 px-[5%] bg-white">
          <div className="max-w-7xl mx-auto">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.15 } } }} className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
              {[
                ...
                <Card key={idx} onClick={() => navigate(item.to)} className="bg-snow p-8 md:p-10 border-forest-green/5 text-center md:text-left">
                  <div className="mb-6 flex flex-col md:flex-row justify-between items-center md:items-start gap-4">
                    ...
                    <div className="p-4 bg-white rounded-2xl group-hover:bg-forest-green group-hover:text-white transition-smooth border border-gray-50">{item.icon}</div>
                  </div>
        ...
        <section id="impact" className="py-24 md:py-32 px-[5%] bg-snow">
          <div className="max-w-6xl mx-auto">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.15 } } }} className="space-y-12">
              {[
                { name: 'Favour', age: '12', story: "Favour is a young, intelligent girl with dreams of becoming a doctor, but those dreams need protecting now before it's too late. Despite everything, Favour consistently tops her class and stays within the top three, but mounting pressure at home threatens to chip away at those marks. Keeping her in school is where it starts.", image: 'https://i.imgur.com/mCCRmmY.jpeg' },
                { name: 'Kelvin', age: '13', story: 'Kelvin has been drawn to wiring and engineering for as long as he can remember. He holds tightly to his dream of becoming an engineer, and right now, that dream needs someone to hold it with him as school fees are mounting and the threat of him being forced to leave school is very real.', image: 'https://i.imgur.com/J2wiqwH.jpeg' },
                { name: 'Raynor', age: '5', story: 'Raynor is currently out of school due to fee arrears. He is five years old and already facing a closed door. Raynor deserves a real shot at the future he was born to have, and school fees are the one bump in the road you can help him cross.', image: 'https://i.imgur.com/eugm70o.jpeg' },
              ].map((item, idx) => (
                <Card key={idx} className="p-0 border-green-50 flex flex-col md:flex-row !rounded-[3rem]" onClick={() => navigate('/impact-stories')}>
                  ...
                  <div className="md:w-2/3 lg:w-3/4 p-8 md:p-12 flex flex-col justify-center">
                    <div className="flex items-center gap-3 mb-4"><span className="w-8 h-[2px] bg-forest-green" /><p className="text-forest-green font-bold text-sm tracking-widest uppercase">{item.name}, Age {item.age}</p></div>
                    <p className="text-lg md:text-2xl text-deep-slate/90 leading-relaxed font-display italic">"{item.story}"</p>
                  </div>
                </Card>
              ))}
            </motion.div>
            <div className="mt-20 flex flex-col sm:flex-row gap-6 justify-center items-center">
              <Button variant="primary" className="px-10 py-4" onClick={() => navigate('/donate')}>Help Change a Life</Button>
              <Button variant="muted" className="px-10 py-4 border-forest-green text-forest-green hover:bg-forest-green/5 font-bold rounded-full border-2" onClick={() => navigate('/impact-stories')}>Discover Impact</Button>
            </div>
          </div>
        </section>
                ...
                {[
                  ...
                  <div key={idx} className="flex flex-col h-full space-y-6">
                    <div className="flex-grow space-y-4">
                      <div className="w-16 h-16 bg-white/30 rounded-full flex items-center justify-center mx-auto mb-6">{item.icon}</div>
                      <h3 className="text-2xl font-display font-bold">{item.title}</h3>
                      <p className="opacity-80 leading-relaxed min-h-[3rem] flex items-center justify-center">{item.desc}</p>
                    </div>
                    <Button variant="primary" className="w-full mt-auto" onClick={() => { if (item.title === 'Volunteer') navigate('/join-volunteer'); else if (item.title === 'Spread the Word') navigate('/share-work'); else navigate('/donate'); }}>
                      {item.btn}
                    </Button>
                  </div>
                ))}
        ...
        <section className="pb-24 px-[5%] text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-xl mx-auto">
            <Button variant="ghost" className="group relative overflow-hidden px-12 py-5 text-xl border-forest-green text-forest-green hover:text-white transition-all duration-500 rounded-full w-full sm:w-auto" onClick={() => navigate('/subscribe')}>
              <span className="relative z-10 flex items-center justify-center gap-3">Never miss an update <ArrowRight className="group-hover:translate-x-2 transition-transform" /></span>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-16 border-b border-white/10">
            <div className="col-span-1 lg:col-span-1">
              <a href="#" className="text-2xl font-display font-bold mb-6 block">fund<span className="text-forest-green">ED</span> futures</a>
              <p className="opacity-70 mb-8 max-w-xs italic">"A better world begins in the mind of a child"</p>
              <div className="flex gap-4">
                {socialLinks.map((social) => (
                  <button key={social.name} type="button" onClick={() => setSocialPopup(social)} aria-label={`Visit our ${social.name}`} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-forest-green transition-colors">
                    <social.icon size={18} />
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); const email = (e.currentTarget.elements.namedItem('footer-email') as HTMLInputElement).value; navigate('/subscribe', { state: { email } }); }}>
                <input id="footer-email" name="footer-email" type="email" placeholder="Email address" className="bg-white/5 border border-white/10 rounded-full px-4 py-2 text-sm focus:outline-none focus:border-forest-green w-full" required />
                <button type="submit" className="bg-forest-green p-2 rounded-full hover:bg-forest-green/80 transition-colors"><ArrowRight size={18} /></button>
              </form>
              <p className="mt-4 text-xs text-forest-green font-bold cursor-pointer hover:underline" onClick={openContact}>Contact: fundedfuturesorg@gmail.com</p>
            </div>
          </div>
          <div className="pt-12 text-center text-sm"><p className="text-forest-green font-bold">© {new Date().getFullYear()} fundED futures. All rights reserved. Based in Nairobi, Kenya.</p></div>
        </div>
      </footer>
      ...
      <AnimatePresence>
        {showCookiePopup && (
          <motion.div initial={{ y: 50, x: '-50%', opacity: 0 }} animate={{ y: 0, x: '-50%', opacity: 1 }} exit={{ y: 50, x: '-50%', opacity: 0 }} className="fixed bottom-6 left-1/2 z-[100] w-[92%] max-w-md bg-white/95 backdrop-blur-xl p-8 rounded-[2rem] border border-gray-200 text-center">
            ...
            <p className="text-sm text-muted-text mb-8 leading-relaxed">We use cookies to enhance your experience and analyze our impact. By clicking "Accept", you consent to our use of cookies.</p>
      ...
      <AnimatePresence>
        {socialPopup && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-deep-slate/60 backdrop-blur-sm flex items-center justify-center px-6" onClick={() => setSocialPopup(null)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[2rem] p-8 md:p-10 max-w-sm w-full text-center" onClick={(e) => e.stopPropagation()}>
              <div className="w-14 h-14 mx-auto mb-6 rounded-full bg-forest-green/10 flex items-center justify-center text-forest-green"><socialPopup.icon size={24} /></div>
              <h4 className="text-lg font-display font-bold mb-3 text-deep-slate">Visit us on {socialPopup.name}</h4>
              <p className="text-sm text-muted-text mb-8 leading-relaxed">{socialPopup.message}</p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Button variant="primary" className="text-xs py-2.5 px-8 w-full sm:w-auto" onClick={() => { window.open(socialPopup.url, '_blank', 'noopener,noreferrer'); setSocialPopup(null); }}>Take Me There</Button>
                <Button variant="ghost" className="text-xs py-2.5 px-8 w-full sm:w-auto border-gray-200" onClick={() => setSocialPopup(null)}>Cancel</Button>
              </div>
            </motion.div>
          </motion.div>
        )}
